...
- 想定する使い方
    - リモートリポジトリからcloneやpullでローカルリポジトリにデータを取り込む
        - 今回はVSCodeのPowerShellを利用
    - VSCodeで編集して、リモートリポジトリに変更内容をpush
    - ローカルで作業して、グローバルに保存しておける
    - グローバルに置いておいて、好きなところから閲覧
    - 鍵を登録すればその端末のローカルリポジトリにも追加可能